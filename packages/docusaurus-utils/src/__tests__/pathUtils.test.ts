...
describe("isNameTooLong", () => {
  it("works", () => {
    ...
    (Object.keys(asserts) as (keyof typeof asserts)[]).forEach((file) => {
      ...
      expect(isNameTooLong(file)).toBe(typeof expected === "boolean" ? expected : expected.apfs);
    });
    ...
    (Object.keys(asserts) as (keyof typeof asserts)[]).forEach((file) => {
      const expected = asserts[file];
      expect(isNameTooLong(file)).toBe(typeof expected === "boolean" ? expected : expected.xfs);
    });
...
describe("shortName", () => {
  it("works", () => {
    ...
    (Object.keys(asserts) as (keyof typeof asserts)[]).forEach((file) => {
      const expected = asserts[file];
      expect(shortName(file)).toBe(typeof expected === "string" ? expected : expected.apfs);
    });
    Object.defineProperty(process, "platform", { value: "win32" });
    (Object.keys(asserts) as (keyof typeof asserts)[]).forEach((file) => {
      const expected = asserts[file];
    ...
    (Object.keys(asserts) as (keyof typeof asserts)[]).forEach((file) => {
      const expected = asserts[file];
      expect(shortName(file)).toBe(typeof expected === "string" ? expected : expected.xfs);
    });
    Object.defineProperty(process, "platform", { value: oldProcessPlatform });
  });
...
describe("toMessageRelativeFilePath", () => {
  it("works", () => {
    jest.spyOn(process, "cwd").mockImplementationOnce(() => path.join(__dirname, ".."));
    expect(toMessageRelativeFilePath(path.join(__dirname, "foo/bar.js"))).toBe("__tests__/foo/bar.js");
  });
});
...
describe("aliasedSitePath", () => {
  it("works", () => {
    ...
    Object.keys(asserts).forEach((file) => {
      expect(posixPath(aliasedSitePath(file, "user/website"))).toBe(asserts[file]);
    });
  });
});
...
describe("addTrailingPathSeparator", () => {
  it("works", () => {
    expect(addTrailingPathSeparator("foo")).toEqual(process.platform === "win32" ? "foo\\" : "foo/");
    expect(addTrailingPathSeparator("foo/")).toEqual(process.platform === "win32" ? "foo\\" : "foo/");
  });
});
