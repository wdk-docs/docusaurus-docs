...
一个**可能的翻译策略**是**使用 Git(或任何其他[VCS](https://en.wikipedia.org/wiki/Version_control))对翻译文件进行**版本控制。

## Tradeoffs {#tradeoffs}

这种策略有好处:

- **易于入门**: just commit the `i18n` folder to Git
- **易于开发人员使用**: Git, GitHub and pull requests are mainstream developer tools
- **免费的** (or without any additional cost, assuming you already use Git)
- **低摩擦**: does not require signing up to an external tool
- **奖励**: contributors are happy to have a nice contribution history

使用 Git 也有一些缺点:

- **对于非开发者来说很难**: they do not master Git and pull-requests
- **专业翻译人员难**: they are used to SaaS translation softwares and advanced features
- **难以维护**: you have to keep the translated files **in sync** with the untranslated files
