...
提供[Blog](blog.mdx)特性，是 Docusaurus 的默认博客插件。

:::caution 一些功能仅用于生产

[feed 特性](../../blog.mdx#feed)通过提取构建输出来工作，并且**仅在生产**中有效。
...
| `feedOptions.type`            | <code><a href="#FeedType">FeedType</a> \| <a href="#FeedType">FeedType</a>[] \| 'all' \| null</code>              | **Required**                                               | 类型 of feed to be generated. Use `null` to disable generation.                                                                                                                                             |
...
| `feedOptions.description`     | `string`                                                                                                          | <code>\`$\{siteConfig.title} Blog\`</code>                 | 描述 of the feed.                                                                                                                                                                                           |
...
Markdown 文档可以使用以下 Markdown [页前](../../guides/markdown-features/markdown-features-intro.mdx#front-matter)元数据字段，在两边用一行`---`括起来。
