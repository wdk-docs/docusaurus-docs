...
例子: When you change any file in `versioned_docs/version-2.6/`, it will only affect the docs for version `2.6`.
...
我们提供了几个导航栏项来帮助您快速设置导航，而不必担心版本化的路由。
...
## 建议实践 {#recommended-practices}

### 仅在需要时才对文档进行版本化 {#version-your-documentation-only-when-needed}
...
### 尽量减少版本的数量 {#keep-the-number-of-versions-small}
...
通过带有`.md`扩展名的相对文件路径引用其他文档，这样 Docusaurus 可以在构建过程中将它们重写为实际的 URL 路径。文件将被链接到正确的相应版本。
...
### 全局或版本化的并置资产 {#global-or-versioned-collocated-assets}

您应该决定像图像和文件这样的资产是按版本分配还是在不同版本之间共享。

如果你的资源需要版本化，把它们放在 docs 版本中，并使用相对路径:
...
如果你的资源是全局的，把它们放在`/static`中，并使用绝对路径:
