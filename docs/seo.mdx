...
通过[网站配置](./configuration.mdx#site-metadata)为整个站点提供全局元属性。元数据将全部呈现在 HTML `<head>`中，使用键值对作为道具的名称和值。`metadata`属性是声明`<meta>`标签的方便快捷方式，但也可以使用`headTags`属性在`<head>`中注入任意标签。
...
Docusaurus 添加了一些开箱即用的元数据。例如，如果您已经配置了[i18n](./i18n/i18n-引言.mdx)，您将得到一个[`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions)替代链接。
...
为了方便，默认的主题`<Layout>`组件接受`title`和`description`作为道具。
